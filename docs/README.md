...
当前页面信息架构以 `video-server` 的普通用户能力为基线：匿名根路由先展示开源、自托管项目介绍；登录后首页承载链接、本地视频与剧本文档入口，一级导航包含首页、下载记录、剧本文档、平台状态与我的。原生认证、下载历史与详情、剧本文档、平台状态、私有封面、短时播放/下载地址和按角色开放的移动管理中心均已接入真实服务；浅色/深色主题在公开、认证与业务页面一键切换并持久化，状态数据在登录后预取并按会话缓存。接口边界见 [`contracts/README.md`](contracts/README.md)。
