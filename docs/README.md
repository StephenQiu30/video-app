...
当前页面信息架构以 `video-server` 的普通用户能力为基线：首页同时承载链接、本地视频与剧本文档入口，一级导航包含首页、下载记录、剧本文档、平台状态与我的。原生认证、下载历史与详情、剧本文档和平台状态的只读真实数据接入已经完成；主题切换位于顶部导航，状态数据在登录后预取并按会话缓存。仍未冻结的创建、上传与文件操作继续 fail closed。接口边界见 [`contracts/README.md`](contracts/README.md)。
