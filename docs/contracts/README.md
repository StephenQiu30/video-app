...
`video-server` 已冻结 42 个 App 操作。本仓库快照位于 `contracts/openapi/video-server.openapi.json`，包含原生认证、来源发现、媒体解析、下载创建、视频与剧本文档上传、下载历史与详情、私有封面、短时下载地址、取消/重试、视频 AI 分析、剧本文档、平台状态，以及下载分析、文件、用户、平台目录和 AI 线路的管理员操作。Flutter 通过生成包 `video_server_api` 接入：
...
认证、登录后只读数据、来源发现、媒体检查、创建下载、视频与剧本文档上传、详情封面、短期文件授权、取消/重试、视频 AI 分析和移动管理中心已经解除阻塞；WebSocket Token 更新、分析报告原生文件导出和剧本文档 AI 分析仍须逐项冻结后才能接入。对应未冻结操作继续 fail closed，不显示模拟远程结果。
