...
每项只允许 `passed`、`failed` 或 `blocked`，并附测试输出、截图、trace、API 请求、生成类型 diff 或人工键盘记录。自动无障碍扫描不能替代人工验收。
...
| `WEB-AC-ID-001` | 安装令牌仅在 server-only BFF，浏览器无泄漏 | bundle/network/启动测试 | pending |
...
产品尚未实现，全部为 `pending`。Plan 001 的细化验收见 `001-链接探测与清晰度目录验收.md`。
