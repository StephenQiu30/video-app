...
### 2026-05-23 阶段二（MVP 功能与工程稳定性）

- 时间：2026-05-23 12:02:20
- 命令与结果：
  - `npm run test` ✅ 通过（13/13）
  - `npm run build` ✅ 通过
  - `npm run lint` ✅ 通过
...
- 覆盖范围：
  - 主页解析失败与 API 错误回显
  - 工作台任务列表错误态、空态、重试入口
  - 任务详情页下载链接获取、失败提示与重试逻辑
  - 未登录路由守卫（`/workbench`、`/tasks/:id`）
  - Playwright 解析 -> 创建 -> 详情链路 E2E 访问与下载入口可见
